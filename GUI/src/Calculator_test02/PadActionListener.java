...
public class PadActionListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) { // 이벤트 처리
        ...
        if (operation.equals("C")) { // 만약 C버튼이라면 계산식 내용을 지워줌
            ...
            num = ""; // 계산 후 num 초기화
            prevOperation = ""; // 연산자 초기화
        } else if (operation.equals("+") || operation.equals("-") || operation.equals("÷") || operation.equals("×")) {
            // 연산자가 - 일 경우 음수 처리
            if (inputSpace.getText().equals("") && operation.equals("-")) { // 음수 처리
    ...
    private void fullTextParsing(String inputText) {
        ...
        for (int i = 0; i < inputText.length(); i++) { // 계산식의 글자를 하나하나 거쳐감
            ...
            if (ch == '-' || ch == '+' || ch == '÷' || ch == '×') {
                ...
                num = num + ch; // 나머지 숫자의 경우 num 문자에 더해줌
            }
        }
        equation.add(num);
        equation.remove("");
            ...
            if (s.equals("+")) {
                ...
                if ((mode.equals("mul") || mode.equals("div")) && (!s.equals("+") && !s.equals("-") && !s.equals("÷") && !s.equals("×"))) {
                    ...
                    if (mode.equals("mul")) {
                        ...
                        // 0으로 나누는 경우 에러 처리
                        if (two == 0) {
                            inputSpace.setText("Error"); // 에러 메시지
                            return 0; // 에러 반환
                        }
                        result = one / two;
                    }
